...
export default function App() {
  ...
  return (
    <div style={{ background: "#020617", minHeight: "100vh", color: "white", fontFamily: "'Apple SD Gothic Neo','Noto Sans KR',sans-serif" }}>
      <div style={{ background: "#0F172A", borderBottom: "1px solid #1E293B", padding: "15px 22px", display: "flex", alignItems: "center", gap: "12px", position: "sticky", top: 0, zIndex: 100 }}>
        <span style={{ fontSize: "22px" }}>🏀</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: "17px", fontWeight: 700 }}>B.PREMIER {CONFERENCES[conf].label} 컨퍼런스</div>
          <div style={{ fontSize: "11px", color: "#475569" }}>{ROSTERS.season} · 갱신 {ROSTERS.updated} · {totalPlayers}명</div>
        </div>
        <div style={{ display: "flex", gap: "6px", background: "#1E293B", borderRadius: "8px", padding: "4px" }}>
          {Object.entries(CONFERENCES).map(([key, { label }]) => (
            <button key={key} type="button" onClick={() => setConf(key)} style={{
              border: "none", cursor: "pointer", borderRadius: "6px", padding: "6px 12px", fontSize: "12px", fontWeight: 700,
              background: conf === key ? "#334155" : "transparent",
              color: conf === key ? "#F8FAFC" : "#64748B",
            }}>{label}</button>
          ))}
        </div>
      </div>

      <div style={{ overflowX: "auto", background: "#0F172A", borderBottom: "1px solid #1E293B", position: "sticky", top: "56px", zIndex: 99 }}>
        <div style={{ display: "flex", minWidth: "max-content" }}>
          {teams.map((t, i) => {
            const active = selId === t.id;
            return (
              <button key={t.id} type="button" onClick={() => setSelId(t.id)} style={{
                padding: "10px 14px", border: "none", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px",
                borderBottom: `3px solid ${active ? t.accent : "transparent"}`,
                background: active ? t.primary + "44" : "transparent",
                color: active ? t.accent : "#64748B", fontSize: "12px", fontWeight: active ? 700 : 400, whiteSpace: "nowrap",
              }}>
                ...
                <span style={{ fontSize: "10px", opacity: 0.7 }}>({t.players.length})</span>
              </button>
            );
          })}
        </div>
      ...
      {teams.length === 0 ? (
        ...
        <>
          <div style={{ padding: "18px 22px", borderBottom: `1px solid ${sel.primary}44`, background: `linear-gradient(90deg, ${sel.primary}33, transparent)`, display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "12px", color: "#64748B" }}>
                2025-26 시즌 · {SORTS[sortBy].label} 순 ·
            ...
            <div style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
              <span style={{ color: "#64748B", fontSize: "12px" }}>정렬</span>
              <select
                value={sortBy}
              ...
              <span style={{ color: "#475569", fontSize: "13px", fontWeight: 600 }}>{players.length}명</span>
            </div>
          </div>
